...
class Instruction(object):
    ...
    def encode(self) -> int:
        ...
        enc = 0
        enc = op_field.insert(self.op.value, enc)
        enc = cond_field.insert(self.cond.value, enc)
        enc = reg_target_field.insert(self.reg_target, enc)
        enc = reg_src1_field.insert(self.reg_src1, enc)
        enc = reg_src2_field.insert(self.reg_src2, enc)
        enc = offset_field.insert(self.offset, enc)
        return enc 
...
def decode(word: int) -> Instruction:
    ...
    op = op_field.extract_signed(word)
    cond = cond_field.extract_signed(word)
